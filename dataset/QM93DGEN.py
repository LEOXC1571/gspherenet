...
def collate_fn(data_batch_list):
    ...
    num_steps_list = torch.tensor(
        [0] + [len(data_batch_list[i]['new_atom_type']) for i in range(len(data_batch_list) - 1)])
    ...
    for key in ['focus', 'c1_focus', 'c2_c1_focus']:
        ...
        atom_offseted = torch.cat([mol_dict[key] for mol_dict in data_batch_list], dim=0) + atom_idx_repeated_offsets[:,
                                                                                            None]
...
class QM93DGEN(InMemoryDataset):
    ...
    def __init__(self, root='../../../../data/xc/molecule_datasets/qm9_3dgen', subset_idx=None, transform=None,
        ...
        if os.path.exists(self.processed_paths[0]):
            ...
            self.process()
        self._indices = subset_idx if subset_idx is not None else range(len(self.atom_type_list))
    ...
    def process(self):
        ...
        for idx in range(len(mols)):
            ...
            if atom_type[0] != 1:
                carbon_idx = np.nonzero(atom_type == 1)
                perm = np.arange(len(atom_type))
                if len(carbon_idx[0]) > 0:
                    first_carbon = carbon_idx[0][0]
                    perm[0] = first_carbon
                    perm[first_carbon] = 0
    ...
    def get_idx_split(self, task):
        ...
        if task == 'rand_gen':
            split_idxs = np.load(os.path.join(self.raw_dir, 'split.npz'))
        elif task == 'gap_opt':
            split_idxs = np.load(os.path.join(self.raw_dir, 'gap.npz'))
        elif task == 'alpha_opt':
            split_idxs = np.load(os.path.join(self.raw_dir, 'alpha.npz'))
        else:
    ...
    def get(self, idx):
        ...
        steps_batch, steps_position, steps_atom_type = torch.empty([0, 1], dtype=int), \
                                                       torch.empty([0, 3], dtype=position.dtype), \
                                                       torch.empty([0, 1], dtype=atom_type.dtype)
        steps_cannot_focus = torch.empty([0, 1], dtype=float)
        steps_dist, steps_angle, steps_torsion = torch.empty([0, 1], dtype=float), torch.empty([0, 1], dtype=float), \
                                                 torch.empty([0, 1], dtype=float)
        idx_offsets = torch.cumsum(torch.arange(len(atom_type) - 1), dim=0)

        for i in range(len(atom_type) - 1):
            partial_con_mat = con_mat[:i+1, :i+1]
            valency_sum = partial_con_mat.sum(dim=1, keepdims=True)
            ...
            if i > 0:
                ...
                if one_step_c1 >= one_step_focus:
                    one_step_c1 += 1
                steps_c1_focus = torch.cat((steps_c1_focus, torch.tensor([one_step_c1, one_step_focus]).view(1, 2) +
                                           idx_offsets[i]))
